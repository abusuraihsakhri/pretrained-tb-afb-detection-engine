...
def check_data_integrity():
    ...
    images = [f for f in img_dir.iterdir() if f.suffix.lower() in {".jpg", ".jpeg", ".png", ".tif", ".tiff"}]
    ...
    # 3. Size constraints (Images must not be 0-byte corrupt files; 0-byte labels are valid negative fields)
    corrupt_images = [f for f in images if f.stat().st_size == 0]
    empty_labels = [f for f in labels if f.stat().st_size == 0]
    
    print_kpi("3. File Size Corruption Verification", len(corrupt_images) == 0, 
              f"Found {len(corrupt_images)} corrupt images | {len(empty_labels)} valid negative background fields.")
    ...
    print("\n-------------------------------------------------------")
    if dir_ok and symmetry_ok and len(corrupt_images) == 0 and valid_format and yaml_ok:
